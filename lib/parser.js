...
TokenParser.prototype = {
  ...
  parseToken: function (token) {
    const self = this;
    const fn = self._parsers[token.type] || self._parsers['*'];
    const match = token.match;
    const prevToken = self.prevToken;
    const prevTokenType = prevToken ? prevToken.type : null;
    const lastState = self.state.length ? self.state[self.state.length - 1] : null;
    let temp;
...
exports.parse = function (swig, source, opts, tags, filters) {
  ...
  const tagOpen = opts.tagControls[0];
  const tagClose = opts.tagControls[1];
  const varOpen = opts.varControls[0];
  const varClose = opts.varControls[1];
  const escapedTagOpen = escapeRegExp(tagOpen);
  const escapedTagClose = escapeRegExp(tagClose);
  const escapedVarOpen = escapeRegExp(varOpen);
  const escapedVarClose = escapeRegExp(varClose);
  const tagStrip = new RegExp(
    '^' + escapedTagOpen + '-?\\s*-?|-?\\s*-?' + escapedTagClose + '$',
    'g'
  );
  const tagStripBefore = new RegExp('^' + escapedTagOpen + '-');
  const tagStripAfter = new RegExp('-' + escapedTagClose + '$');
  const varStrip = new RegExp(
    '^' + escapedVarOpen + '-?\\s*-?|-?\\s*-?' + escapedVarClose + '$',
    'g'
  );
  const varStripBefore = new RegExp('^' + escapedVarOpen + '-');
  const varStripAfter = new RegExp('-' + escapedVarClose + '$');
  const cmtOpen = opts.cmtControls[0];
  const cmtClose = opts.cmtControls[1];
  const anyChar = '[\\s\\S]*?';
  ...
  function parseVariable (str, line) {
    const lexedTokens = lexer.read(utils.strip(str));
    let parser;
    let out;
  ...
  function parseTag (str, line) {
    let lexedTokens, parser, chunks, tagName, tag, args, last;
...
exports.compile = function (template, parents, options, blockName) {
  let out = '';
  const tokens = utils.isArray(template) ? template : template.tokens;
