...
exports.Swig = function (opts) {
   ...
   * @return {object|undefined} options    Template function and tokens.
   * @private
   */
  ...
  this.parse = function (source, options) {
    ...
    const locals = getLocals(options);
    const opt = {};
    let k;
   ...
   * @param  {object} blocks Current template that needs to be mapped to the  parent's block and token list.
   * @return {array}
   * @private
   */
  function remapBlocks (blocks, tokens) {
    return utils.map(tokens, function (token) {
      const args = token.args ? token.args.join('') : '';
  ...
  this.precompile = function (source, options) {
    const tokens = self.parse(source, options);
    const parents = getParents(tokens, options);
    let tpl;
  ...
  this.compile = function (source, options) {
    const key = options ? options.filename : null;
    const cached = key ? cacheGet(key, options) : null;
    let context;
    let contextLength;
    let pre;
  ...
  this.run = function (tpl, locals, filepath) {
    const context = getLocals({ locals: locals });
    if (filepath) {
